  ...
  auth: router({
    ...
    signup: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Set session cookie
        const cookieOptions = getSessionCookieOptions(ctx.req);
        ctx.res.cookie(COOKIE_NAME, JSON.stringify({ userId }), cookieOptions);
  ...
  requests: router({
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await notifyOwner({
          title: 'طلب جديد',
          body: `طلب صيانة جديد من ${ctx.user.name}`,
        });
    ...
    getMyRequests: protectedProcedure
      .query(async ({ ctx }) => {
        return await db.getUserRequests(ctx.user.id);
      }),
    ...
    // Get pending requests for technicians
    getPending: technicianProcedure
      .query(async ({ ctx }) => {
        return await db.getPendingRequests(ctx.technician.city);
      }),
    ...
    accept: technicianProcedure
      .input(z.object({ requestId: z.number() }))
      .mutation(async ({ ctx, input }) => {
        await db.assignRequestToTechnician(input.requestId, ctx.technician.id);
        return { success: true };
      }),
    ...
    updateStatus: technicianProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const request = await db.getRequestById(input.requestId);
        if (!request || request.technicianId !== ctx.technician.id) {
    ...
    getMyAssignedRequests: technicianProcedure
      .query(async ({ ctx }) => {
        return await db.getTechnicianRequests(ctx.technician.id);
      }),
  }),
